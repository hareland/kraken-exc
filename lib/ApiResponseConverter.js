...
function parseCurrencyPair(pair) {
  // find base currency by known prefixes/passthru
  for (let i = 3; i <= 5; i++) {
    const baseCode = pair.slice(0, i);
    const baseCurrency = convertFromKrakenCurrencyCode(baseCode);

    // If baseCurrency is found and differs from the input baseCode, it's valid
    if (baseCurrency !== baseCode || constants.PASSTHRU_CURRENCIES.includes(baseCurrency)) {
      const quoteCode = pair.slice(i);
      const quoteCurrency = convertFromKrakenCurrencyCode(quoteCode);
      return { baseCurrency, quoteCurrency };
    }
  }

  // If no valid split is found, return an error
  return { error: `Invalid currency pair: ${pair}` };
}
/**
 * convertKrakenTrade
